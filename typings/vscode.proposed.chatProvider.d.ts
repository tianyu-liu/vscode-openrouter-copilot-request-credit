...
// Vendored copy of VS Code's proposed `chatProvider` API (microsoft/vscode,
// src/vscode-dts/vscode.proposed.chatProvider.d.ts). Kept OUT of the tsconfig
// build (tsconfig includes only src/) — used here as the authoritative
// reference for the runtime fields the extension relies on:
//
//   - LanguageModelChatInformation.configurationSchema  — declaring this on a
//     model makes VS Code's model picker render the native "Thinking Effort"
//     submenu (any schema property with `group: 'navigation'` becomes a
//     primary picker action).
//   - ProvideLanguageModelChatResponseOptions.modelConfiguration — the resolved
//     per-model configuration (validated against the schema), delivered to
//     provideLanguageModelChatResponse. The picked effort lands here as
